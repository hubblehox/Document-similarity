    ...
    def _calculate_similarities(self):
        ...
        for i in range(len(self.vectors)):
            for j in range(i+1, len(self.vectors)):
                cosine_similarity = np.dot(self.vectors[i], self.vectors[j]) / (np.linalg.norm(self.vectors[i]) * np.linalg.norm(self.vectors[j]))
...
def index():
    if request.method == 'POST':
        ...
        similar_docs = ds.get_similar_documents(3)
...
if __name__ == '__main__':
    openai.api_key =""
    app.run()
# <label for="text3">Text 3:</label><br>
